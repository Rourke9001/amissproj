 ...
 *
 * <p>Wraps the project's {@link DB} helper. Reads return the same fallback the game
 * has always used when no row matches; writes propagate {@link SQLException} for the
...
public class UserStatsRepository {
    ...
    public UserStatsRepository(DB db) {
        this.db = db;
    }

    // ---- reads -------------------------------------------------------------

    public int getEducation(String name) throws SQLException {
        return db.queryForInt("SELECT education from tbluserstats where name = ?", -1, name);
    }

    public int getEduprog(String name) throws SQLException {
    ...
    public String getWork(String name) throws SQLException {
    ...
    public String getHappiness(String name) throws SQLException {
        return db.queryForString("SELECT happiness FROM tbluserstats WHERE name = ?", null, name);
    }

    // ---- writes ------------------------------------------------------------

    public void updateEducation(String name, int education) throws SQLException {
        db.update("UPDATE tbluserstats set education = ? where name = ?", education, name);
    }

    public void updateEduprog(String name, int eduprog) throws SQLException {
        db.update("Update tbluserstats set eduprog = ? where name = ?", eduprog, name);
    }

    public void incrementWork(String name) throws SQLException {
        db.update("UPDATE tbluserstats SET work = work + ? WHERE name = ?", 1, name);
    }

    public void incrementHappiness(String name) throws SQLException {
    ...
    public void insertNewStats(String name) throws SQLException {
    ...
    public void resetStats(String name) throws SQLException {

 ...
 * the save-scoped services, an authenticated player reaches their own save fine, {@code
 * /api/highscores} stays public, and CORS preflight is only honoured for an allowed origin.
 * {@link BoardController}, {@link PlayerController} and {@link HighscoresController} are
 * loaded together purely so every scenario fits in one test class; none of their behaviour
 * beyond routing/security is under test here (see each controller's own {@code
 * *ControllerTest}).
 */
